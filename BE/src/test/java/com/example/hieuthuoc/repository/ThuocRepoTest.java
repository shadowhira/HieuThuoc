...
public class ThuocRepoTest {
    ...
    void testFindByTenThuoc_WithExistingName() {
        ...
        Mockito.when(thuocRepo.findByTenThuoc(tenThuoc, pageable)).thenReturn(expectedPage);
        ...
        Mockito.verify(thuocRepo, Mockito.times(1)).findByTenThuoc(tenThuoc, pageable);
    }
    ...
    void testExistsByMaThuoc_WithExistingCode() {
        ...
        Mockito.when(thuocRepo.existsByMaThuoc(maThuoc)).thenReturn(true);
        ...
        // Assert
        assertTrue(result);
        Mockito.verify(thuocRepo, Mockito.times(1)).existsByMaThuoc(maThuoc);
    }
    ...
    void testExistsByMaThuoc_WithNonExistingCode() {
        ...
        Mockito.when(thuocRepo.existsByMaThuoc(maThuoc)).thenReturn(false);
    ...
    void testExistsByTenThuoc_WithExistingName() {
        // Arrange
        String tenThuoc = "Paracetamol 500mg";
        Mockito.when(thuocRepo.existsByTenThuoc(tenThuoc)).thenReturn(true);
        ...
        // Assert
        assertTrue(result);
        Mockito.verify(thuocRepo, Mockito.times(1)).existsByTenThuoc(tenThuoc);
    }
    ...
    void testSearch_WithMultipleCriteria() {
        // Arrange
        String keyWord = "Para";
        String loaiThuocStr = "Giảm đau";
        String nhaSanXuatStr = "ABC";
        String danhMucThuocStr = "kháng sinh";
        ...
        Mockito.when(thuocRepo.search(
            keyWord, loaiThuocStr, nhaSanXuatStr, danhMucThuocStr,
        ...
        Mockito.verify(thuocRepo, Mockito.times(1)).search(
            keyWord, loaiThuocStr, nhaSanXuatStr, danhMucThuocStr,
            minGiaBan, maxGiaBan, tenDoiTuong, trangThai, pageable
        );
    }
